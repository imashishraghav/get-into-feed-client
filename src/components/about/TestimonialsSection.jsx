...
//
// import { client } from "@/sanity/lib/client"; // Updated Sanity client path
      ...
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          ...
          className={`transition-colors duration-300 ${
            i < validRating ? "fill-primary text-primary" : "fill-slate-100 text-slate-200"
          }`}
        />
      ))}
    </div>
...
const FallbackAvatar = ({ name, className }) => {
  ...
  return (
    <div className={`rounded-full bg-gradient-to-br from-background to-slate-200 border border-border flex items-center justify-center text-secondary font-bold shadow-sm ${className}`}>
      {initial}
    </div>
  );
};
...
export default function TestimonialsSection({ testimonials = fallbackTestimonials }) {
  ...
  return (
    <section className="relative w-full bg-background py-16 md:py-24 overflow-hidden selection:bg-primary/20 selection:text-secondary">
      ...
      <motion.div 
        style={{ y: bgDrift }}
        className="absolute top-1/4 right-[-10%] w-[600px] h-[600px] bg-gradient-to-tr from-primary/5 to-transparent rounded-full blur-[140px] pointer-events-none z-0 gpu-accelerated" 
      />
...
function FeaturedCard({ testimonial }) {
  ...
  return (
    <motion.div ref={cardRef} style={{ y, opacity }} className="w-full gpu-accelerated">
      <div className="relative group bg-white rounded-[2.5rem] p-8 md:p-14 lg:p-16 border border-border shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-500 hover:shadow-[0_30px_60px_-15px_rgba(46,209,178,0.15)] overflow-hidden">
        
        {/* Decorative Quote mark & Glow */}
        <Quote className="absolute -top-8 -left-8 md:top-8 md:left-8 text-background w-48 h-48 z-0 rotate-180 transition-transform duration-700 group-hover:scale-110" />
        <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-primary to-secondary opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        ...
        <div className="relative z-10 flex flex-col lg:flex-row gap-10 lg:gap-16 items-center lg:items-start text-center lg:text-left">
          ...
          <div className="flex-1">
            ...
            <blockquote className="text-2xl md:text-3xl lg:text-4xl font-heading font-extrabold text-navy leading-snug tracking-tight mb-8">
              "{testimonial.feedback}"
            </blockquote>
            
            <div className="flex items-center justify-center lg:justify-start gap-4">
              <div className="shrink-0 w-16 h-16 relative rounded-full overflow-hidden shadow-sm border border-border bg-background">
                {testimonial.imageUrl ? (
                  <Image src={testimonial.imageUrl} alt={testimonial.name} fill className="object-cover" sizes="(max-width: 768px) 64px, 64px" />
                ) : (
                  <FallbackAvatar name={testimonial.name} className="w-full h-full text-xl" />
                )}
              </div>
              <div className="text-left">
                <h4 className="text-lg font-heading font-bold text-navy">{testimonial.name}</h4>
                <p className="text-sm font-sans font-semibold text-slate-600">
                  {testimonial.role} <span className="text-primary mx-1">•</span> {testimonial.company}
                </p>
              </div>
            </div>
          </div>
  ...
  return (
    <motion.div ref={cardRef} style={{ y, opacity }} className="h-full gpu-accelerated">
      <motion.div 
        ...
        className="relative h-full bg-white rounded-3xl p-8 border border-border shadow-sm transition-all duration-300 ease-out hover:border-primary/30 hover:shadow-[0_20px_40px_-10px_rgba(46,209,178,0.12)] flex flex-col group overflow-hidden"
      >
        <Quote className="absolute top-6 right-6 text-background w-16 h-16 rotate-180 transition-transform duration-500 group-hover:scale-110" />

        <div className="relative z-10 flex-1 flex flex-col">
          {renderStars(testimonial.rating)}
          
          <blockquote className="font-sans text-slate-600 text-lg font-medium leading-relaxed mb-8 flex-1">
            "{testimonial.feedback}"
          </blockquote>
          
          <div className="flex items-center gap-4 pt-6 border-t border-border/50">
            <div className="shrink-0 w-12 h-12 relative rounded-full overflow-hidden border border-border bg-background">
              {testimonial.imageUrl ? (
                <Image src={testimonial.imageUrl} alt={testimonial.name} fill className="object-cover" sizes="(max-width: 768px) 48px, 48px" />
              ) : (
                <FallbackAvatar name={testimonial.name} className="w-full h-full text-base" />
              )}
            </div>
            <div>
              <h4 className="text-base font-heading font-bold text-navy">{testimonial.name}</h4>
