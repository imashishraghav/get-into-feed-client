...
export default async function SingleServiceDetailPremiumPage({ params }) {
  ...
  return (
    <div className="bg-navy min-h-screen pt-28 pb-20 font-sans selection:bg-primary/30 selection:text-white relative overflow-hidden">
      
      {/* Global Ambient Glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[100vw] h-[60vh] bg-gradient-to-b from-primary/[0.05] to-transparent rounded-full blur-[150px] pointer-events-none -z-10" />

      {/* --- BACK BUTTON --- */}
      <div className="max-w-[1100px] mx-auto px-6 md:px-8 relative z-10 mb-12">
        <Link href="/services" className="inline-flex items-center gap-2 text-sm font-bold text-slate-400 hover:text-primary transition-colors uppercase tracking-wider w-max group bg-white/5 px-5 py-2.5 rounded-full border border-white/5 hover:border-primary/30">
        ...
        <div className="flex flex-col lg:flex-row items-center lg:items-start gap-12">
          ...
          <div className="flex-1 text-center lg:text-left">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 mb-6">
              ...
              <span className="text-xs font-bold tracking-widest uppercase text-primary">Premium Service</span>
            </div>
            
            <h1 className="font-heading text-4xl md:text-6xl lg:text-7xl font-extrabold tracking-tight mb-8 text-white leading-[1.1] text-balance">
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400">{service.title}</span>
            </h1>
            
            <p className="text-slate-400 text-lg md:text-xl font-medium leading-relaxed max-w-2xl mx-auto lg:mx-0 text-balance">
          ...
          <div className="w-48 h-48 md:w-64 md:h-64 shrink-0 relative">
            ...
            <div className="w-full h-full rounded-[2.5rem] bg-gradient-to-br from-[#131b24] to-[#0a0f14] border border-white/10 flex items-center justify-center shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative z-10 transform lg:rotate-3 hover:rotate-0 transition-transform duration-500">
              {service.iconUrl ? (
                <img src={service.iconUrl} alt={service.title} className="w-24 h-24 object-contain drop-shadow-glow" />
              ) : (
                <Target size={80} className="text-primary drop-shadow-glow" strokeWidth={1} />
              )}
            </div>
          </div>
        </div>
      ...
      <section className="py-24 relative border-t border-b border-white/5 bg-[#05080c]">
        <div className="max-w-[1100px] mx-auto px-6 md:px-8 relative z-10">
          ...
          <div className="text-center mb-20">
            <h2 className="font-heading text-4xl md:text-5xl font-extrabold text-white mb-6 text-balance">Our <span className="text-primary">Holistic</span> Approach</h2>
          ...
          <div className="grid md:grid-cols-2 gap-8 lg:gap-12 relative">
            ...
            {[
              ...
              <div key={i} className="bg-[#0b1015]/80 backdrop-blur-xl p-8 md:p-10 rounded-[2rem] border border-white/10 hover:border-primary/50 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(46,209,178,0.1)] relative z-10 group">
                <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 blur-[40px] rounded-full pointer-events-none group-hover:bg-primary/15 transition-colors"></div>
                
                <div className="flex items-center gap-6 mb-6">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#1a2530] to-[#111820] flex items-center justify-center text-primary border border-white/5 shadow-inner">
                    {step.icon}
                  </div>
                  <h3 className="font-heading text-2xl font-bold text-white leading-tight">{step.title}</h3>
                </div>
      ...
      <section className="py-24 relative">
        <div className="max-w-[1100px] mx-auto px-6 md:px-8">
          <div className="bg-gradient-to-br from-[#131b24] to-[#0a0f14] rounded-[3rem] p-8 md:p-16 border border-primary/20 shadow-[0_0_50px_rgba(46,209,178,0.05)] flex flex-col lg:flex-row items-center gap-12 lg:gap-20 overflow-hidden relative">
            
            {/* Glowing orb */}
            <div className="absolute top-0 right-0 w-[300px] h-[300px] bg-primary/10 blur-[100px] rounded-full pointer-events-none"></div>

            <div className="flex-1 relative z-10">
              <h2 className="font-heading text-3xl md:text-5xl font-extrabold text-white mb-6 text-balance">
                Why We Should Be Your <span className="text-primary">#1 Choice?</span>
              </h2>
              ...
              <div className="space-y-6">
                {[
                  ...
                  <div key={i} className="flex gap-4">
                    ...
                    <div>
                      <h4 className="font-heading text-white font-bold text-lg mb-1">{item.title}</h4>
            ...
            <div className="lg:w-5/12 w-full relative z-10 hidden md:block">
               <div className="aspect-square rounded-full border border-dashed border-primary/30 flex items-center justify-center p-8 relative animate-[spin_60s_linear_infinite]">
                  <div className="w-full h-full rounded-full border border-primary/20 flex items-center justify-center p-8">
                     <div className="w-full h-full rounded-full bg-primary/10 blur-xl"></div>
                  </div>
               </div>
               <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                 <Award size={100} className="text-primary drop-shadow-[0_0_30px_rgba(46,209,178,0.6)]" strokeWidth={1} />
               </div>
            </div>
      ...
      <section className="py-12 relative">
        <div className="max-w-[1100px] mx-auto px-6 md:px-8">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div>
              <h2 className="font-heading text-4xl md:text-5xl font-bold text-white mb-4 text-balance">Our <span className="text-primary">Work</span></h2>
              <p className="text-slate-400 text-lg max-w-lg text-balance">Real results for real brands. Peek into our success stories.</p>
            </div>
            <Link href="/case-studies" className="text-primary font-bold hover:underline underline-offset-4 mb-2">View All Portfolio →</Link>
          </div>
            ...
            {[
              ...
              <div key={i} className="group relative rounded-[2rem] overflow-hidden aspect-video bg-slate-900 cursor-pointer">
                ...
                <div className="absolute inset-0 p-8 flex flex-col justify-end">
                  <span className="bg-primary text-[#022c22] text-xs font-bold px-3 py-1 rounded-full w-max mb-4 uppercase tracking-widest">{work.tag}</span>
                  <h3 className="font-heading text-2xl md:text-3xl font-bold text-white mb-2 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">{work.title}</h3>
                  <div className="flex items-center text-primary font-semibold opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
      ...
      <section className="py-24 relative max-w-[800px] mx-auto px-6 md:px-8">
        <div className="text-center mb-16">
          <h2 className="font-heading text-4xl font-bold text-white mb-4 text-balance">Some of The <span className="text-primary">Questions</span></h2>
          <p className="text-slate-400 text-balance">Everything you need to know about working with us.</p>
        </div>
        ...
        <div className="space-y-4">
          {[
            ...
            <details key={i} className="group bg-[#0b1015]/60 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-sm [&_summary::-webkit-details-marker]:hidden">
              <summary className="font-heading flex items-center justify-between p-6 cursor-pointer text-white font-bold text-lg select-none outline-none">
                {faq.q}
      ...
      <section className="py-12 pb-24 relative max-w-[1100px] mx-auto px-6 md:px-8 text-center">
         <h2 className="font-heading text-3xl md:text-5xl font-extrabold text-white mb-8 leading-tight text-balance">
           Let us transform your ideas into <br className="hidden md:block"/>
           <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-[#6ee7b7]">captivating visuals</span> that elevate your brand.
         </h2>
         <Link href="/contact" className="font-heading inline-flex items-center gap-2 bg-primary text-[#022c22] font-bold py-4 px-10 rounded-xl hover:bg-[#4df287] transition-all shadow-[0_0_30px_rgba(46,209,178,0.3)] hover:-translate-y-1 text-lg">
